...
public interface ProjectApiDocs {
    ...
    Response<TraceHipMap> hitMap(TraceSearchRequest request);

    @Operation(summary = "상태 코드 분포 조회", description = "프로젝트의 상태 코드 분포를 조회합니다.")
    Response<StatusCodeDistribution> statusCodeDistribution(TraceSearchRequest request);
