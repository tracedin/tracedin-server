...
public interface ProjectApiDocs {
    ...
    Response<ProjectKey> createProject(UserPrincipal currentUser, CreateProjectRequest request);
    ...
    Response<List<NodeResponse>> serviceNodes(UserPrincipal currentUser, String projectKey);

    @Operation(summary = "네트워크 토폴로지 조회", description = "프로젝트의 네트워크 토폴로지를 조회합니다.")
    Response<NetworkTopology> networkTopology(UserPrincipal currentUser, String projectKey);

    @Operation(summary = "히트맵 조회", description = "프로젝트의 히트맵을 조회합니다.(5분 별 트레이스의 응답시간 분포)")
    Response<List<EndTimeBucket>> hitMap(
            UserPrincipal currentUser, String projectKey, String serviceName);

    @Operation(summary = "프로젝트 멤버 추가", description = "프로젝트에 멤버를 추가합니다.")
    Response<Void> addMember(UserPrincipal currentUser, Long projectId, AddMemberRequest request);

    @Operation(summary = "프로젝트 멤버 삭제", description = "프로젝트의 멤버를 삭제합니다.")
    Response<Void> removeMember(UserPrincipal currentUser, Long projectMemberId);

    @Operation(summary = "프로젝트 멤버 권한 변경", description = "프로젝트의 멤버 권한을 변경합니다.")
    Response<Void> changeRole(
            UserPrincipal currentUser, Long projectMemberId, MemberRole targetRole);
}
